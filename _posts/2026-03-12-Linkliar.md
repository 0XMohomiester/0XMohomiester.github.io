...
tags: [Mobile, IOS, CTF, Mobile Hacking Lab, Cyber Security, Penetration Testing] 
---
...
All of these observations confirm that the crash is caused by **memory corruption** during HTTP header processing. This leads us to the next stage of the analysis, where we focus on understanding how the corruption occurs in memory and how the application **manages function calls and stack frames** during execution.
...
As shown in the previous figure, we can observe a **simplified representation of the stack frame** during the execution of a function. When a function is called, the system stores important control information on the stack. This includes:
...
When the vulnerable application connects to this server and processes the response headers, the oversized header name is **copied into the fixed-size stack buffer inside the header parsing routine**. Because the copy operation does not **enforce proper bounds checking**, the header name exceeds the allocated buffer size and **overwrites adjacent stack memory**.

![BufferOverFlow](../assets/images/Linkliar/12.jpg)
*Start a Simple HTTP Server*

![BufferOverFlow](../assets/images/Linkliar/13.jpg)
*Triggering the Crash*

After triggering the crash, we inspected the processor registers using the `register read` command in the LLDB debugger. The output revealed that the `program counter (PC)` had been overwritten with a value containing the pattern `0x41414141`, which corresponds to the ASCII character `A`.

![Overwritten_PC](../assets/images/Linkliar/14.jpg)
*Overwritten program counter (PC)*

This behavior allows us to eventually redirect execution to the application’s flag() function. According to the challenge hints, solving the challenge in the intended way requires redirecting the program’s execution flow to this function. This exploitation process will be demonstrated in the following section.

## Exploiting Buffer Overflow Vulnerability

After confirming the existence of the **stack-based buffer overflow** and determining that **32 bytes are required to overwrite the return address**, the next step is to craft an input that redirects execution to the application's `flag() function`.

Using the `LLDB` debugger, we executed the following command:

```shell
dis -n flag
```

This command displays the disassembly of the `flag()` function and reveals its address within the loaded binary. Since the binary is loaded in memory during runtime, the displayed address already reflects the correct runtime location of the function.

![Flag_Function](../assets/images/Linkliar/15.jpg)
*Find a Flag Function*

we can construct the exploit payload. Since our analysis showed that the saved return address is reached after 32 bytes, the payload structure becomes:

```
[32 bytes padding] + [flag() address]
```
![Flag_Function](../assets/images/Linkliar/16.jpg)
*Crafting the Exploit Payload*

we append the address of the `flag()` function in `little-endian format`, which is required because ARM64 systems store addresses in little-endian byte order.

To confirm that the exploit correctly redirects execution, we set a **breakpoint at the address of the flag()** function in the debugger. When the malicious header is processed and the vulnerable function returns, the overwritten return address causes execution to jump directly to the flag() function.

Hitting the breakpoint confirms that the program counter (PC) has been successfully redirected, proving that the buffer overflow allows full control over the application's execution flow.

![Redirect_Execution](../assets/images/Linkliar/17.jpg)
*Redirect Execution to the Flag Function*

During the reverse engineering phase in **Ghidra**, we observed that the application checks for a specific debug configuration before performing certain operations. 

![Check_for_specific_debug](../assets/images/Linkliar/18.jpg)
*Check for specific debug configuration*

To bypass this condition and force the application to interact with our custom server, we modified the application's debug configuration by setting the key `DebugURL` in NSUserDefaults to the value:
`http://192.168.1.7:8000` using the following command:

```shell
expr (void)[[NSUserDefaults standardUserDefaults] setObject:@"http://192.168.1.7:8000" forKey:@"DebugURL"]
```

![Check_for_specific_debug](../assets/images/Linkliar/19.jpg)
*Change the debug configuration*

Once execution is successfully redirected to the **flag() function**, the application sends the challenge flag to the server. By intercepting the network traffic using **Burp Suite**, we can observe the outgoing request containing the flag in the request body.

![Check_for_specific_debug](../assets/images/Linkliar/20.jpg)
*Retrieving the Flag*

## Conclusion

In this challenge, we identified a **buffer overflow vulnerability** in the way the application processes HTTP headers. By analyzing the app’s behavior, debugging it at runtime, and reviewing the code statically, we were able to understand how the memory corruption occurs.

After determining the correct offset, we successfully overwrote the return address and redirected execution to the `flag()` function. We also adjusted the app’s configuration to ensure it communicates with our controlled server, allowing us to capture the flag.

This challenge shows how unsafe memory handling can lead to serious vulnerabilities, and how combining debugging, reverse engineering, and exploitation techniques can help fully understand and exploit them.

## 
References: 

  - [ARM stack overflow Challenges](https://azeria-labs.com/part-3-stack-overflow-challenges/)
  - [ARM32 stack overflow](https://azeria-labs.com/stack-overflow-arm32/)
  - [ARM Stack Limit Registers](https://interrupt.memfault.com/blog/using-psp-msp-limit-registers-for-stack-overflow)
  - [ARM Exploit Development - Stack Overflow](https://blogs.night-wolf.io/arm-exploit-development-part-1-stack-overflow)

Thanks for reading!

If you found this write-up helpful, feel free to connect with me on [Linkedin](https://www.linkedin.com/in/0xmohomiester/) for more content on mobile security, reverse engineering, and penetration testing.
